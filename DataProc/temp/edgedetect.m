...
% H = fspecial('average',50);                 % create the filter of average
% img2 = imfilter(img,H,'replicate');        % get the smoothed image
% pixel_400=smooth(img(400,:),10);
% diff_pixel=smooth(diff(smooth(pixel_400,100)),10);
% pos= diff_pixel==max(diff_pixel);
% threshold=img(400,pos);

load('img2')
info.Width=size(img,2);
info.Height=size(img,1);
threshold=find_threshold(img,1);

%%
ref_color=0;                                  % the backbround lightness is 67
bin_mask = zeros(size(img,1),size(img,2));
...
biggest_Blob=zeros(size(img,1),size(img,2));
