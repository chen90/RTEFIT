...
if handles.Rdrift_on
    if handles.ntime==1 %only one time slice ntime=1
        ...
        fig=figure('Name','Remove drift from Flux loop signal');% figure flux vs flux loop No
        set(gcf,'units','normalized','position',[0.2 0.2 0.6 0.6]);
        plot(fluxloop_No,COILS_t_tmp,':o','LineWidth',1.5,'MarkerSize',6); %unit Wb
        l{1}=['t=',num2str(t),' s'];
        xlabel('$Loop-No$','interpreter','Latex');
        ylabel('$\rm\phi (\textit{Wb})$','interpreter','Latex'); %unit Wb
        title(['Flux Loop Data,',l{1}])
       ...
       %% linear fitting of the data to remove the drift from the raw data
        ...
        figure(fig);
        hold on;plot(fluxloop_No,COILS_t_tmp,'r:*','LineWidth',1.5,'MarkerSize',6); %unit Wb
        legend('Flux Raw Data','Flux Data Processed','location','SouthWest','fontsize',16);
    ...
    %% Not remove the drift and check the flux loop data, if only one time slice
     if handles.ntime==1 %only one time slice ntime=1
         if handles.Smooth==1 %smooth the raw data for all the flux loops
            ...
            %plot the flux loops raw data    
            fig=figure('Name','Original flux loop signal');% figure flux vs flux loop No
            set(gcf,'units','normalized','position',[0.2 0.2 0.6 0.6]);
            plot(fluxloop_No,COILS_t_tmp,':o','LineWidth',1.5,'MarkerSize',6); %unit Wb
            l{1}=['t=',num2str(t),' s'];
            xlabel('$Loop-No$','interpreter','Latex');
...
if handles.ntime==1
    %plot the interpolate data for the flux loop 27    
    figure(fig);
    hold on;
    plot(fluxloop_No(27),COILS_t_tmp(27),'r*','LineWidth',1.5,'MarkerSize',6); %unit Wb
end
