...
if strcmpi(varargin, 'contour')
    if gfile.sibry<0
        ...
        hold on;
        surf(rmesh_refine,zmesh_refine,min(min(flux_surface(:,1:80)))*ones(size(rmesh_refine))*1.05,ones(size(rmesh_refine))*1.0);
        surf(rmesh_refine(:,1:80),zmesh_refine(:,1:80),flux_surface(:,1:80));
        colormap(flipud(hot));
        shading interp;
        grid off
        box off
        view([0,0,1]);
        zlim([min(min(flux_surface(:,1:80)))*1.1,0]);
        caxis([min(min(flux_surface(:,1:80)))*1.1,0]);
    else
...
hold on;
contour(rmesh_refine,zmesh_refine,flux_efit_refine',150,'Color',[0.5 0.5 0.5],'linewidth',1.5);   

% v=[1 2 3 4 6 8 10 20 30 40 50 60 80 100]/10000;
% [C,h]=contour(rmesh_refine,zmesh_refine,flux_efit_refine',v);   
% clabel(C,h,v);
