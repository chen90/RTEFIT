...
% handles    structure with handles and user data (see GUIDATA)
    ...
    Run_output = ssh2_simple_command('192.168.20.22','public','public',['cd ',RTEFIT_path,'&& ./runefit ',num2str(ISHOT),' ',num2str(ntime)]);
    delete(h);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    elseif handles.Data_source==1 % select time interval from experiment 
        ...
        try
            [ip,t_signal] = exl50db(ISHOT,'IP','0:12:0.05'); %plasma current in unit PLASMA_unit(kA), t_signal in unit s, channels from ip01 to ip04 in different toroidal locations
        catch
        ...
        [ipf1,t2] = exl50db(ISHOT,'IPF1','0:12:0.05');
        [ipf6,t3] = exl50db(ISHOT,'IPF6','0:12:0.05');
        [ipf3,t4] = exl50db(ISHOT,'IPF3','0:12:0.05');
        [ipf4,t5] = exl50db(ISHOT,'IPF4','0:12:0.05');
        [mwi,t6] = exl50db(ISHOT,'MWI_NE001','0:12:0.05');
%         [blockgas,t7] = exl50db(ISHOT,'BLOCKGAS','0:12:0.05');
        [m1pin,t8] = exl50db(ISHOT,'M1_Pin','0:12:0.05');
        [m1pout,t9] = exl50db(ISHOT,'M1_Pout','0:12:0.05');
        [m2pin,t10] = exl50db(ISHOT,'M2_Pin','0:12:0.05');
        [m2pout,t11] = exl50db(ISHOT,'M2_Pout','0:12:0.05');
        [m3pin,t12] = exl50db(ISHOT,'M3_Pin','0:12:0.05');
        [m3pout,t13] = exl50db(ISHOT,'M3_Pout','0:12:0.05');
...
% end
% 
% [FLUX003,t2] = exl50db(ISHOT,'FLUX003','0:12:0.05');
% [FLUX007,t3] = exl50db(ISHOT,'FLUX007','0:12:0.05');
% [FLUX017,t4] = exl50db(ISHOT,'FLUX017','0:12:0.05');
% [FLUX026,t5] = exl50db(ISHOT,'FLUX026','0:12:0.05');
% 
% fig=figure(1);
